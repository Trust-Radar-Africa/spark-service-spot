...
const buildApiPath = (path: string, backend: ApiBackend, baseUrl: string): string => {
  ...
  if (backend === 'json-server') {
    // JSON Server v1.x serves resources at root: /users, /jobs, etc.
    // Strip /api/admin/ or /api/ prefixes
    if (normalizedPath.startsWith('api/admin/')) {
      normalizedPath = normalizedPath.slice(10); // /api/admin/candidates -> candidates
    } else if (normalizedPath.startsWith('api/')) {
      normalizedPath = normalizedPath.slice(4); // /api/candidates -> candidates
